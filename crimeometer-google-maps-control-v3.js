/*
*    crimeometer-google-maps-control-v3.js
...
function initMap() {
    ...
    // Load parameters
    var script_tag = document.getElementById('crimeometer-google-maps-control-v3');
    var riskmap_index = script_tag.getAttribute("riskmap-index");
    ...
    const origin_lat = parseFloat(data_lat); 
    const origin_lng = parseFloat(data_lon);
    ...
    if (data_tk != '') {
        ...
        var TILE_URL = 'https://tiles.crimeometer.com/get-tile-v2?index={index}&tk={tk}&z={z}&x={x}&y={y}'

        layer = new google.maps.ImageMapType({
            name: 'layerID',
            getTileUrl: function(coord, zoom) {
                var url_tile = TILE_URL
                    .replace('{index}', riskmap_index)
                    .replace('{x}', coord.x)
                    .replace('{y}', coord.y)
                    .replace('{z}', zoom)
                    .replace('{tk}', data_tk);
